...
def cmake_build_project(arch, config):
    ...
    pathlib.Path(f"{project_paths['dotnet-proj']}/native/{arch['dotnet-folder']}").mkdir(parents=True, exist_ok=True)
    for filename in os.listdir(f"{project_paths['dotnet-proj']}/native/{arch['dotnet-folder']}"):
        os.remove(os.path.join(f"{project_paths['dotnet-proj']}/native/{arch['dotnet-folder']}", filename))

    for filename in os.listdir(f"./build/{arch['build-folder']}/install/bin/{config}"):
        shutil.copy(f"./build/{arch['build-folder']}/install/bin/{config}/{filename}", f"{project_paths['dotnet-proj']}/native/{arch['dotnet-folder']}")
   
    for filename in os.listdir(f"{project_paths['dotnet-proj']}/native/{arch['dotnet-folder']}"):
        arguments = filename.split(".");
        dll_name = arguments[0].split("_")[0]
        src = f"{project_paths['dotnet-proj']}/native/{arch['dotnet-folder']}/{filename}"
        dst = f"{project_paths['dotnet-proj']}/native/{arch['dotnet-folder']}/{dll_name}.{arguments[-1]}"
...
def dotnet_pack_project(config, is_local):
    dotnet_generate_version(f"{project_paths['dotnet-proj']}/Version.props", is_local)
...
def dotnet_run_native_tests(arch, config):
    for test_name in gpu_test_list:
        subprocess.run(f"./build/{arch['build-folder']}/Tests/DiligentCoreAPITest/{config}/DiligentCoreAPITest.exe \
                    --mode={gpu_tests_api} \
...
def dotne_restore_nuget_package(config, src_dir):
    shutil.rmtree(f"{src_dir}/RestoredPackages", ignore_errors=True) 
    subprocess.run(f"dotnet restore --no-cache {src_dir}", check=True)

def dotnet_build_tests(config):
    dotnet_generate_version(f"{project_paths['dotnet-test']}/Version.props")
    dotnet_run_native_tests(config_windows["x64"], config)
    dotnet_copy_assets(f"{project_paths['gpu-test-assets']}", f"{project_paths['dotnet-test']}/Assets")
    dotnet_copy_nuget_package(f"{project_paths['dotnet-proj']}/bin/{config}", f"{project_paths['dotnet-test']}/LocalPackages")
    dotne_restore_nuget_package(config, project_paths['dotnet-test'])

def dotnet_run_tests(config, arch):
...
def main():
    ...
    for arch in config_windows.keys():
        cmake_build_project(config_windows[arch], args.configuration)
    ...
    if args.dotnet_tests:
        ...
        dotnet_build_tests(args.configuration)
        for arch in config_windows.keys():
            dotnet_run_tests(args.configuration, arch)
